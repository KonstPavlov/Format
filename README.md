...
отправка заявок на e-mail (SMTP) **уже настроена** и работает сразу.
...
### Сменить почту / SMTP (необязательно)

Реквизиты по умолчанию заданы прямо в `docker-compose.yml`. Чтобы использовать
другой ящик, переопределите переменные — экспортом в шелле или через корневой
файл `.env` (Docker Compose подхватит его автоматически):
...
В составе проекта есть сервис **Caddy** — он публикует сайт наружу на портах
`80/443`, автоматически получает и продлевает бесплатные сертификаты Let's Encrypt
и перенаправляет HTTP → HTTPS. Фронтенд и бэкенд наружу не публикуются — доступ
только через Caddy.

Настроен домен `format-remont.ru` и `www.format-remont.ru` (файл `Caddyfile`).

**Перед запуском на чистом Ubuntu VPS:**

1. Направьте A-записи `format-remont.ru` и `www.format-remont.ru` на IP сервера.
2. Убедитесь, что порты `80` и `443` открыты (не заняты другим nginx/apache).
...
Caddy сам выпустит сертификаты при первом обращении. Проверить:
`https://format-remont.ru`.

Чтобы использовать другой домен — отредактируйте первую строку `Caddyfile`.
